...
# 	fig.write_image(file='hh.png', format='.png')
	fig.show()
...
if __name__ == '__main__':
	...
	pretrained = load_model(args.path, embed_dim = 64, n_customer = args.n_customer, n_encode_layers = 3)
	print(f'model loading time:{time()-t1}s')
	t2 = time()
	data = generate_data(device, 2000, 20)
...
# 	print(data[1].shape, data[2].shape, data[3].shape)
	...
	# print('costs:', costs)
	print('mean_costs:',torch.mean(costs, dim = 0))
